...
def get_file_path(filename):
    """Возвращает правильный путь к файлу рядом со скриптом bot.py (в папке test/)"""
...
def fetch_free_cis_proxies():
    """Автоматический бесплатный сбор свежих СНГ прокси (RU, KZ, BY)"""
    proxies = []
...
def setup_browser_with_proxy(p, is_github):
    ...
    # 2. Прокси из Secret в GitHub (Webshare)
    ...
    # 4. Автоматический подбор живого СНГ прокси
...
def run_dungeon_bot():
    ...
    with sync_playwright() as p:
        ...
        try:
            # Загружаем cookies.txt из папки test/
            ...
            if resp and resp.status in [403, 502, 503]:
                print(f"❌ Блокировка сайта (HTTP {resp.status}).")
        ...
        try:
            ...
            try:
                ...
                if login_btn.is_visible(timeout=5000):
                    print("⚠️ Требуется вход. Авторизуюсь...")
                    login_btn.click()
                    human_sleep(2, 3)
                    ...
                    print("✅ Вход выполнен, жду прогрузки...")
                    human_sleep(6, 8)
        ...
        while True:
            try:
                ...
                if kanji_element.count() > 0:
                    ...
                    print("✅ Клик по иероглифу 寺!")
                else:
                    print("Иероглиф не найден. Выход.")
                    break 
                ...
                try:
                    ...
                    battle_btn.click()
                    print("⚔️ Кнопка боя нажата!")
                except Exception as ex:
                    print(f"Кнопка боя не найдена: {ex}")
